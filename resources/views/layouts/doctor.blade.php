...
<head>
    ...
    <title>Dokter - Kurnia Care</title>
...
<body class="bg-gray-100 text-gray-800 overflow-x-hidden">
    ...
    <div x-data="{
        ...
        <aside x-cloak :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <div class="p-6 border-b border-emerald-600 flex justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-white">Kurnia Care</h1>
                    <p class="text-sm text-emerald-100">Dokter Panel</p>
                </div>
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('doctor.dashboard') }}" @click.prevent="goTo('{{ route('doctor.dashboard') }}')"
    ...
    : 'hover:bg-emerald-600' }}">
                ...
                <a href="{{ route('doctor.appointments.index') }}"
                    @click.prevent="goTo('{{ route('doctor.appointments.index') }}')" class="block px-4 py-3 rounded-lg transition
                        {{ request()->routeIs('doctor.appointments.*')
    ? 'bg-emerald-600 text-white font-semibold'
    : 'hover:bg-emerald-600' }}">
                    Jadwal Pasien
                </a>
                ...
                <a href="{{ route('doctor.appointments.history') }}"
                    @click.prevent="goTo('{{ route('doctor.appointments.history') }}')" class="block px-4 py-3 rounded-lg transition
                       {{ request()->routeIs('doctor.appointments.history')
                     ? 'bg-emerald-600 text-white font-semibold'
                     : 'hover:bg-emerald-600' }}">
                    Riwayat Tindakan
                </a>
                ...
                <a href="{{ route('doctor.medical-notes.index') }}"
                    @click.prevent="goTo('{{ route('doctor.medical-notes.index') }}')" class="block px-4 py-3 rounded-lg transition
                  {{ request()->routeIs('doctor.medical-notes.index')
                 ? 'bg-emerald-600 text-white font-semibold'
                    : 'hover:bg-emerald-600' }}">
                    Catatan Medis
                </a>
                ...
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <h2 class="text-lg md:text-xl font-semibold truncate">
                            @yield('title', 'Dashboard Dokter')
                        </h2>

                        <p class="text-xs md:text-sm text-gray-500 truncate">
                            Jadwal dan tindakan pasien Kurnia Care
                        </p>
                    </div>
