...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div
                class="bg-white rounded-xl shadow-sm p-5 border-l-4 {{ $appointment->status === 'dibatalkan' ? 'border-red-500' : 'border-emerald-500' }}">
            ...
            <div
                class="bg-white rounded-xl shadow-sm p-5 border-l-4 {{ $payment && $payment->status === 'ditolak' ? 'border-red-500' : 'border-amber-400' }}">
            ...
            <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-emerald-500">
                <p class="text-sm text-gray-500">Nominal DP</p>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <p class="font-semibold">{{ $patient->child_name ?? '-' }}</p>
                </div>

                <div>
                    <p class="text-gray-500">Umur</p>
                    <p class="font-semibold">{{ $patient->child_age ?? '-' }} tahun</p>
                </div>

                <div>
                    <p class="text-gray-500">Berat Badan</p>
                    <p class="font-semibold">{{ $patient->child_weight ?? '-' }} kg</p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="font-semibold">{{ $patient->drug_allergy ?? '-' }}</p>
                </div>

                <div class="md:col-span-2">
                    <p class="text-gray-500">Riwayat Perdarahan</p>
                    <p class="font-semibold">{{ $patient->bleeding_history ?? '-' }}</p>
                </div>

                <div class="md:col-span-2">
                    <p class="text-gray-500">Riwayat Operasi</p>
                    <p class="font-semibold">{{ $patient->surgery_history ?? '-' }}</p>
                </div>

                <div class="md:col-span-2">
                    <p class="text-gray-500">Riwayat Penyakit</p>
                    <p class="font-semibold">{{ $patient->disease_history ?? '-' }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <p class="font-semibold">{{ $patient->father_name ?? '-' }}</p>
                </div>

                <div>
                    <p class="text-gray-500">Nama Ibu</p>
                    <p class="font-semibold">{{ $patient->mother_name ?? '-' }}</p>
                </div>

                <div>
                    <p class="text-gray-500">Instagram</p>
                    <p class="font-semibold">{{ $patient->instagram ?? '-' }}</p>
                </div>

                <div>
                    <p class="text-gray-500">Facebook</p>
                    <p class="font-semibold">{{ $patient->facebook ?? '-' }}</p>
                </div>

                <div>
                    <p class="text-gray-500">Sumber Informasi</p>
                    <p class="font-semibold">{{ $patient->information_source ?? '-' }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <p class="font-semibold">{{ $payment->payment_method ?? 'Transfer Bank' }}</p>
                </div>
                ...
                <div>
                    <p class="text-gray-500">Bukti Pembayaran DP</p>
                ...
                <div class="md:col-span-2 rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                    ...
                    <p class="text-sm text-gray-600 mt-3">
                        Pelunasan dilakukan setelah tindakan khitan selesai.
                    </p>
                </div>

                @if ($payment && $payment->status === 'ditolak' && $payment->rejection_reason)
                ...
                @elseif ($appointment->status === 'selesai')
                    <div class="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                        <p class="text-emerald-700 font-semibold">
                            DP sudah diterima. Jadwal khitan sudah dikonfirmasi.
                        </p>

                        <p class="text-sm text-gray-600 mt-1">
                            Sisa pelunasan dilakukan setelah tindakan khitan selesai.
                        </p>
                    </div>
                @elseif ($payment->status === 'ditolak')
                    <div class="rounded-lg bg-red-50 border border-red-200 p-4">
                        ...
                        <a href="{{ route('user.payments.edit', $appointment) }}"
                            class="inline-block mt-3 px-5 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700">
                            Upload Ulang Bukti DP DP
                        </a>
                    </div>
                @elseif ($payment->status === 'diterima')
                    <div class="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                        <p class="text-emerald-700 font-semibold">
                            Pembayaran sudah diterima dan tidak bisa diubah.
                        </p>
                    </div>
                    ...
                    @foreach ($appointment->medicalNotes as $note)
                        <div class="border border-gray-200 rounded-lg p-4">
                            <p class="text-sm text-gray-500">
                                Status: {{ ucfirst(str_replace('_', ' ', $note->action_status)) }}
                            </p>
